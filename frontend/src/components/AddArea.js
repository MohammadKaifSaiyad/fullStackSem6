...
function AddArea({areaList, areaData, setAreaData, fetchUserAreas}) {
  ...
  return (
    <div className="bg-zinc-300 h-full flex flex-row w-full">
      <ToastContainer/>
      <dvi className="w-3/6">
        <form class="max-w-sm mx-12 my-12">
          <div class="mb-5">
            ...
            <input type="text" name="area" disabled={false} value={areaData.area} onChange={handelChange} id="area_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Enter area name" required/>
          </div>
          <div class="mb-5">
          ...
          <button type='button' onClick={handleAddArea} disabled={delay || !(areaData.name!='')} class="text-white bg-gray-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">{edit?"Edit":"Add"}</button>
        </form>
      </dvi>
      <div className="w-3/6 flex flex-col p-10">
        <button onClick={handleEditArea} className="text-white self-center justiry-self-center bg-gray-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm h-10 sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">{editArea? "Enable Add Department": "Edit Department"}</button>
        {
          editArea && 
          <div className="flex mt-5 bg-gray-400 flex-col">
            <div className="text-xl m-2">Department List</div>
            <List className="bg-gray-500 m-2">
              {
                areaList?areaList.areas.map(area=><div className="flex"><ListItem className="m-1 p-1 w-full flex flex-row"> <ListItemPrefix className="w-2/6 text-lg ">{area.name}</ListItemPrefix> <div className="w-3/6 text-md">Location: {area.location}</div></ListItem> <MdEdit size={25} className="self-center mx-0.5 cursor-pointer hover:border-2 hover:border-gray-700 hover:rounded-md" onClick={()=>{handleAreaEdit(area)}}/> <MdDelete onClick={()=>{handleDeleteArea(area)}} size={25} className="mx-0.5 self-center cursor-pointer hover:border-2 hover:border-gray-700 hover:rounded-md"/></div>)
                :null
              }
            </List>
          </div>
