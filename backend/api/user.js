...
router.post('/login',async(req, res)=>{
    ...
    await loggedinuser.findOne({userEmail: req.body.email})
        .then((user)=>{
...
router.post('/getuserdata',checkCookies,(req, res)=>{
    ...
    loggedinuser.findById(req.body.user_id)
    .then(userData=>{
        res.json({
            status: 'SUCCESS',
            user_data:{
...
router.post('/createloggedinuser',async (req, res)=>{
    ...
    await usersignup.findOne({userEmail: email})
                    .then(async user=>{
                ...
                if(canDelete){ await usersignup.deleteOne({userEmail:req.body.email}); }
                res.json({
                    status: 'SUCCESS',
